const Version = '2026-06-17 01:41:21 (Merged Upstream)';
...
function 随机路径(完整节点路径 = "/") {
	...
	const 随机数 = (randomBytes[0] % 3) + 1;

	const shuffled = [...常用路径目录];
	const randomIndices = new Uint32Array(shuffled.length);
	crypto.getRandomValues(randomIndices);
	for (let i = shuffled.length - 1; i > 0; i--) {
		const j = randomIndices[i] % (i + 1);
		[shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
	}
	const 随机路径 = shuffled.slice(0, 随机数).join('/');
...
function 替换星号为随机字符(内容) {
	...
	return 内容.replace(/\*/g, () => {
		let s = '';
		const lengthArray = new Uint8Array(1);
		crypto.getRandomValues(lengthArray);
		const length = (lengthArray[0] % 14) + 3;
		const randomValues = new Uint8Array(length);
		crypto.getRandomValues(randomValues);
		for (let i = 0; i < length; i++) {
			s += 字符集[randomValues[i] % 字符集.length];
		}
		return s;
	});
}
